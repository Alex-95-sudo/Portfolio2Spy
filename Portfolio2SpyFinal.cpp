...
private:
    ...
    void moveGuards() {
        for (int i = 0; i < (int)guards.size(); i++) {
            ...
            // Guard catches player by moving onto them
            if (player.row == newR && player.col == newC) {
...
public:
    // Overloaded Constructors for Level 
    ...
    void play() {
        ...
        while (running) {
            ...
            if (action.length() != 1) {
                cout << "Invalid input. Use WASD or type 'inspect'.\n";
                continue;
            }

            char move = tolower(action[0]);
            cin.ignore(10000, '\n');
